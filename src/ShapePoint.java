...
public class ShapePoint extends Shape implements Serializable {	
	...
	// 점(x1, y1)과 점(x2, y2)를 연결하는 직선
  	...
  	// Line 생성자
	public ShapePoint(int x1, int y1, int x2, int y2, Color color)
  	{
	...
	// 직선을 그린다.
  	public void draw(Graphics g)
  	{
    	...
    	// 해석 :: 마우스 포인터가 가리키는 위치 x1, y1, x2, y2에 선을 긋는다.
    	//line = new Line2D.Double(x1, y1, x2, y2); 
    	g2d.setColor(getColor());											// 색을 설정하고
	
    	//g2d.draw(line);			
  	
    	// 해석 :: 그리는 곳의 굵기를  10으로 조절한다.
